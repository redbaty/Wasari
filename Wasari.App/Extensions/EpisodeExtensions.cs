...
public static class EpisodeExtensions
{
    private static bool FilterEpisodes(Ranges? range, int? number)
    {
        ...
        if (flag && range.Minimum.HasValue) flag = number >= range.Minimum;

        if (flag && range.Maximum.HasValue) flag = number <= range.Maximum;
    ...
    {
        return episodes.Where(episode => FilterEpisodes(episodesRange, episode.Number) && FilterEpisodes(seasonsRange, episode.SeasonNumber));
    }
