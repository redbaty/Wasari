...
internal static class Program
{
    private static async Task<int> Main()
    {
        ...
        serviceCollection.Configure<FFmpegResolutionPresets>(c => { c.Presets.Add("4k", FFmpegResolution.FourK); });
        var serviceProvider = serviceCollection.BuildServiceProvider();

        var environmentOptions = serviceProvider.GetService<IOptions<EnvironmentOptions>>();

        if (environmentOptions?.Value?.Features is { } features) await Console.Error.WriteLineAsync($"Available environment features: {features.Select(i => $"\"{i.Type}\"").Aggregate((x, y) => $"{x}, {y}")}");
